...
After this change a contributor can run the daily replay (and the batch replay) directly against the new parquet market-data root at `/Users/liyijing/Projects/Trading/market-data/`, while the legacy `exec/data/{TSE,OTC}Quote.YYYYMMDD` text dumps remain available as an explicit compatibility source. The user-visible payoff is dramatic: the cold per-day load drops from roughly sixteen seconds of pure-Python text parsing to roughly one hundred and thirty milliseconds of column-projected parquet reads, the twenty-day history window cold build drops from about two hundred and fifty seconds to about three seconds, and the on-disk footprint of one trading day shrinks from roughly two and a half gigabytes to roughly two hundred and forty megabytes. Parquet is accepted by satisfying its own source contract, not by producing byte-identical reports against the text feed.

Concretely, a novice can demonstrate the result by running the daily-replay command with the default parquet source on a date such as `20260326` and observing shorter `[TIMING]` lines plus valid generated CSV reports under `log/`. They can also run `scripts/compare_text_vs_parquet.py` to classify cross-source cumulative-volume differences as audit evidence.

The non-goals are equally important. This plan does not change strategy logic, does not promote the new `group-ver20260408.csv` universe to default, does not touch the live (Redis) provider, and does not require deleting legacy text files unless a separate retirement decision is made.
...
- [x] (completed 2026-04-08T00:00:00Z) M3 — Create `src/tw_signal_engine/market_data/parquet_history_loader.py` exposing `load_parquet_history_window(market_type, date, root)` returning a `HistoryWindow` of the same shape as the existing text loader. Add unit tests under `tests/unit/test_parquet_history_loader.py` for parquet-source invariants: schema, status-equivalent filtering, chronological history nodes, non-negative volume, and deterministic output. Real-data text comparisons are diagnostic only.
...
- [x] (completed 2026-04-08T00:00:00Z) M5 — Wire a `--data-source {text,parquet}` CLI flag into `src/tw_signal_engine/cli/run_daily_replay.py` and `src/tw_signal_engine/cli/run_batch_replay.py`. Plumb the flag through `run_daily_replay` so it picks `FileReplayProvider` or `ParquetReplayProvider`, and through history loading so it picks `load_history_window` or `load_parquet_history_window`.
- [x] (completed 2026-04-20) M6 — Replace the strict A/B parity gate with the source-contract gate. `report_trades.csv` may differ between text and parquet when source semantics differ. Required parquet checks are schema validation, explicit missing-file/missing-symbol guards, valid market mapping, non-negative volumes, chronological replay order, stable output for identical inputs, and report provenance via `DataSource`.
...
- [ ] (pending) M8 — Decide the long-term status of the text path (`fallback` versus `retire`). Do not delete text loaders, providers, caches, tests, or the `--data-source text` branch as part of parquet acceptance.
- [ ] (pending) M9 — Repoint the example commands and `exec/cfg/parameter.cfg` notes to `/Users/liyijing/Projects/Trading/market-data/`, archive the design report and this plan to `docs/exec-plans/completed/`, and write the `Outcomes & Retrospective` entry after the source-contract checks are green on representative dates.
...
- 2026-04-20 — Policy update: strict parquet-vs-text output parity is no longer the acceptance gate. The evidence below shows that several residual deltas are caused by source-data semantics (`00*` omission, history quantity semantics, zero-quantity/status snapshots), not conversion arithmetic. Parquet is now treated as a first-class replay source with its own truth contract, while text remains an explicit compatibility source. Cross-source diffing remains useful but diagnostic.
...
- 2026-04-08 — Five-date M1 cumulative-volume parity summary (`scripts/compare_text_vs_parquet.py`):
  ...
  All non-text-only deltas are the small auction-batch differences described in the previous bullet; the rest are entirely the 00-symbol gap. Under the current policy these are classified diagnostics, not a zero-diff acceptance gate.
...
- Decision: Implement the new path additively and preserve text as an explicit compatibility source until a separate retirement decision is made.
  Rationale: Parquet is now verified by its own source contract, not by strict text-output parity. Keeping `FileReplayProvider`, `iterate_market_file`, `parse_format6_replay_rows`, `parse_history_trades`, `merge_market_streams`, `build_volume_caches`, and `rolling_history` available behind `--data-source text` preserves fallback behavior without making text the canonical truth for parquet.
  Date/Author: 2026-04-08 / planning author (per phase 5 of the design report).

- Decision: Treat parquet and text as separate truth sources.
  Rationale: A/B replay evidence shows structural source-data differences, including omitted `00*` symbols, history quantity drift, and zero-quantity/status snapshot differences. Cross-source equality is diagnostic evidence, not a release gate.
...
A "provider" is anything implementing `MarketDataProvider` from `src/tw_signal_engine/market_data/providers.py`. The interface is two methods: an optional `start_listener()` (used by Redis live) and an `iterate_ticks() -> Iterator[MarketTick]`. Daily and batch CLIs default to `ParquetReplayProvider`; `FileReplayProvider` remains available for `--data-source text`.
...
The second milestone (M1) builds the cross-source diagnostic harness. `scripts/compare_text_vs_parquet.py` accepts `--date YYYYMMDD`, loads the legacy text path via `iterate_market_file("TSE", date, "exec/data/")` summed by symbol, loads the parquet path via `pyarrow.parquet.read_table` with column projection on `["symbol", "tradeVolume"]` and the parquet replay filters (`matchFlag == "Y"`, regular-session time, and `tradeVolume > 0`), then prints classified differences. Repeat for `TPEX` against `OTC`. Run on five representative dates (`20260319`, `20260320`, `20260323`, `20260325`, `20260326`) and paste the per-date results into the `Surprises & Discoveries` section of this plan. Differences are audit findings; they are not parquet acceptance failures unless they violate the parquet source contract.
...
The fifth milestone (M4) implements the replay provider. Create `src/tw_signal_engine/market_data/parquet_replay_provider.py` with a `ParquetReplayProvider` class implementing `MarketDataProvider`. Its `__init__` takes `otc_date`, `tse_date`, `root` (the parquet root, default `/Users/liyijing/Projects/Trading/market-data/tick-data/`), `tick_filter`, `prev_day_limit_up`, and `num_tracker` — the same shape as `FileReplayProvider`. Its `iterate_ticks()` reads both `TWSE/<tse_date>.parquet` and `TPEX/<otc_date>.parquet` with the `PARQUET_REPLAY_COLUMNS` projection and the parquet replay filters, optionally pushing down `("symbol", "in", list(tick_filter))` when `tick_filter` is non-empty (pyarrow accepts a Python set's list). It tags each table with a `market` column (`"TSE"` or `"OTC"`), concatenates them via `pa.concat_tables`, and sorts by `time` ascending. It then iterates `RecordBatch` chunks of ten thousand rows each (never `to_pandas()` on the full table, per R7 in the design report's risk register, because materializing eight and a half million rows by thirty-six columns balloons to roughly two gigabytes of pandas memory) and yields one `MarketTick` per row. Each row is converted with `to_int_price`, `_convert_raw_time_to_us`, and the same `prev_limit_up` and `volatility_pause` post-processing that `merge_market_streams` does today. Add `tests/unit/test_parquet_replay_provider.py` with source-contract cases for chronological order, `tick_filter` pushdown, price conversion, status filters, volatility flags, and deterministic output. Real-data comparisons against `iterate_market_file` are diagnostic only.

The sixth milestone (M5) adds the CLI flag. Edit `src/tw_signal_engine/cli/run_daily_replay.py` and `src/tw_signal_engine/cli/run_batch_replay.py` to expose `--data-source` with `choices=("text", "parquet")`. The CLIs now default to `"parquet"`. Edit `run_daily_replay()` in `src/tw_signal_engine/replay/replay_session.py` to accept `data_source`; when `data_source == "parquet"`, swap `FileReplayProvider` for `ParquetReplayProvider` and swap `load_history_window` for `load_parquet_history_window`. The provider parameter that already exists (used by the live path, see the redis-live remediation plan) takes precedence: if a caller passes `provider=...` explicitly, do not override it with the parquet provider.
...
The seventh milestone (M6) is the source-contract gate. Run parquet replay on representative dates and verify that schema checks pass, missing Symbols files are guarded, parquet files map `TSE -> TWSE` and `OTC -> TPEX`, emitted ticks are chronological, volumes are non-negative, price and time conversion are stable, repeated runs over identical parquet inputs produce the same reports, and `report_trades.csv` includes `DataSource=parquet`. Optional A/B runs against `--data-source text` should be recorded as diagnostics and classified as expected source differences or potential regressions.
...
The ninth milestone (M8) decides the long-term text-path status. If the decision is `fallback`, keep the text loaders/providers/tests and document operational support expectations. If the decision is `retire`, create a separate retirement plan that owns deletion of the text parsers, `FileReplayProvider`, rolling caches, exclusive tests, and the `--data-source text` branch. Do not tie this decision to strict parquet-vs-text report equality.
...
Expected per invocation: a summary with `expected source differences` and `potential regressions` counts plus a top-N classified difference table. Review potential regressions manually; do not require zero cross-source deltas.
...
Expected: the second replay invocation prints noticeably shorter `[TIMING] getTickData` and `[TIMING] readFileMerged` lines than the first (look for sub-second versus tens of seconds), writes `report_trades.csv` with `DataSource=parquet`, and completes without source-contract violations. The comparison script is diagnostic; review its classifications instead of requiring an empty diff.
...
M8 — full validation after the text-path status decision.

    uv run pytest tests -q
    uv run ruff check src tests
    uv run mypy src

Expected: zero failures, zero ruff violations, zero mypy errors. If text retirement is separately approved, run the same validation after that deletion plan lands.
...
Expected: the active index loses the entry for this plan; the completed index gains it; the three top-level docs reflect parquet as default, text as compatibility/fallback, and source-specific truth contracts.
...
First, `uv run pytest tests -q` is green from a fresh `uv sync` with no optional extras. Second, `uv run python -m tw_signal_engine.cli.run_daily_replay --date 20260326 ... --data-source parquet` (or, after M7, the same command with no flag) prints `[TIMING] getTickData OTC` and `[TIMING] getTickData TSE` lines whose totals are below five seconds combined, where the legacy text path printed totals above sixty seconds. Third, parquet schema checks fail clearly when required columns are missing or mistyped, and history loading rejects negative `tradeVolume`. Fourth, `report_trades.csv` includes `DataSource=parquet` for parquet replay output. Fifth, invoking the daily replay against `--date 20260407` (a tick-data date with no Symbols file) prints the `[GUARD] Skipping 20260407` line and exits zero, instead of crashing with `FileNotFoundError`. Sixth, `scripts/compare_text_vs_parquet.py --date 20260326 --market TSE` remains available as a diagnostic classifier and is not used as a zero-diff release gate.
...
If the source-contract gate in M6 fails, do not advance. Decide whether the violation is in schema validation, price conversion (revisit `parquet_io.to_int_price`), row filters (revisit `matchFlag == "Y"`, regular-session time, and `tradeVolume > 0`), market mapping, merge order, volume handling, or report provenance. Record the finding in `Surprises & Discoveries` and patch the offending file. Re-run M6 from scratch.
...
If `transactionVolume-delta mismatches: 0`, the hypothesis holds and no legacy executions are lost — they are simply batched into wider snapshots. If there are mismatches, paste the first ten rows of `mismatches` into `Surprises & Discoveries` and classify them as expected source differences or potential regressions.
...
In `src/tw_signal_engine/replay/replay_session.py` (M5), extend the `run_daily_replay` signature with one new parameter:
    ...
    def run_daily_replay(
        ...
        data_source: str = "text",   # "text" or "parquet"; CLIs pass parquet by default
    ) -> list[TradeRecord]: ...
...
- 2026-04-20 — Replaced strict parquet-vs-text parity with the source-separated truth contract. M6 now requires parquet schema checks, explicit guards, valid mapping, non-negative volumes, chronological order, deterministic output, and `DataSource` report provenance. Cross-source diffing remains available as diagnostics through `scripts/compare_text_vs_parquet.py`.
